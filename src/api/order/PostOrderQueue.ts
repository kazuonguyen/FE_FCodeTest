...
// Return true to check if the order is in queue
// Don't use state because it's not updated yet
export default async function PostOrderQueue(id_order: string, dispatch: Dispatch<AnyAction>) {
    let result = false;

    try {
        const res = await fetchLogistics(`protected/order/queue?idorder=${id_order}`, {
            method: "POST",
            withToken: true,
        });
        ...
        if (res.status === 200) {
            result = true;
            dispatch(setSuccess(`Đã xác nhận đơn hàng ${id_order} đang chờ!`));
