...
import { statusShipmentList } from "@/functions/constant";

export default function FilterShipmentShip({ isActived }: { isActived: boolean }) {
            ...
            <div
                ...
                onClick={() => {
                    if (!isActived) setShowDropdown(!showDropdown);
                        ...
                        dispatch(clearFilterShipmentByDeliveryStatus());
                        setShowDropdown(false);
                    }
                }}
            >
                ...
                >
                    {filterShipmentByDeliveryStatus !== ""
                        ? handleFilterStatus(filterShipmentByDeliveryStatus)
                        : "Trạng thái GH"}
                </p>
            ...
            {showDropdown === true && (
                ...
                >
                    <div>
                        ...
                        {statusShipmentList &&
                            statusShipmentList.map((item, index) => {
                                return (
                                    <button
                                        type="button"
                                        key={index}
                                        ...
                                        onClick={() => {
                                            dispatch(setFilterShipmentByDeliveryStatus(item));
                                            setShowDropdown(false);
                                        }}
                                        style={{
                                            fontSize: "15px",
                                    ...
                                    >
                                        <p>
                                            {item === "arrived" && "Đã giao hàng"}
                                            {item === "unarrived" && "Chờ giao hàng"}
                                        </p>
                                    </button>
                                );
                            })}
