...
export default function FilterShipmentPrice({ isActived }: { isActived: boolean }) {
    ...
    const [leftPrice, setLeftPrice] = useState<string>(
        formatNumber(String(filterShipmentByLeftPrice)),
    );
    const [rightPrice, setRightPrice] = useState<string>(
        formatNumber(String(filterShipmentByRightPrice)),
    );
    ...
    useEffect(() => {
        setLeftPrice(formatNumber(String(filterShipmentByLeftPrice)));
        setRightPrice(formatNumber(String(filterShipmentByRightPrice)));
    }, [filterShipmentByLeftPrice, filterShipmentByRightPrice]);
                    ...
                    <div className="flex flex-col items-center justify-center">
                        ...
                        <div className="relative flex flex-row items-center justify-center w-full px-6 py-5">
                            <ReactSlider
                                ...
                                max={2147483646}
                                value={[
                                    filterShipmentByLeftPrice as number,
                                    filterShipmentByRightPrice as number,
                                ]}
                                ...
                                onChange={(value, index) => {
                                    if (index === 0) {
                                        dispatch(setFilterShipmentByLeftPrice(value[0]));
                                    } else {
                                        dispatch(setFilterShipmentByRightPrice(value[1]));
                                    }
                                }}
                                onSliderClick={(value) => {
                                    // Calculate left and right value from click position to update value in store
                                    const left = Math.abs(filterShipmentByLeftPrice - value);
                                    const right = Math.abs(filterShipmentByRightPrice - value);
                                    if (left < right) {
                                        dispatch(setFilterShipmentByLeftPrice(value));
                                    } else {
                                        dispatch(setFilterShipmentByRightPrice(value));
                                    }
                                }}
                                minDistance={0}
                        ...
                        >
                            <button
                                ...
                                onClick={() => {
                                    dispatch(clearFilterShipmentByPrice());
                                    setLeftPrice("0");
                                    setRightPrice("2,147,483,646");
                                    setShowDropdown(false);
                                }}
